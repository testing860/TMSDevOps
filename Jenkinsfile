pipeline {
    ...
    stages {
        stage('Checkout from GitHub') {
            steps {
                // Use workspace path from Jenkins credentials
                dir("${JENKINS_WORKSPACE}") {
                    checkout([
                        $class: 'GitSCM',
                        branches: [[name: '*/main']],
                        userRemoteConfigs: [[
                            // URL from Jenkins credentials, not hardcoded!
                            url: "${GITHUB_REPO_URL}",
        ...
        stage('Create Secure Environment File') {
            steps {
                dir("${JENKINS_WORKSPACE}") {
                    // Create the .env file for the .NET app using Jenkins secrets
                    sh '''
                        cat > .env << 'EOF'
